...
class HealthEndpointTest {
    ...
    @Test
    void springTokenCsrfIsDeliberatelyDisabledInFavourOfTheClientHeaderCheck() {
        // Spring's token-based CSRF filter is turned off on purpose: CSRF defence comes from the
        // X-Horizon-Client header check added with auth. If this test fails, someone re-enabled Spring's
        // CSRF (or added a second chain that has it); decide that together with the header check rather
        // than just deleting the assertion.
        // An unauthenticated POST already answers 401 because the CSRF 403 is re-dispatched to /error and
        // turned into a 401, so the status alone cannot show CSRF is off. Check every chain itself.
        assertThat(securityFilterChains)
                .isNotEmpty()
                .flatExtracting(SecurityFilterChain::getFilters)
